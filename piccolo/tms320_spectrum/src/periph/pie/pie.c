...
static void pie_init_vector_table(
  void
)
{
    int16   i;
...
)
{
    ...
    InitPieCtrl(); // reset all regs

    IER = 0x0000;
    IFR = 0x0000;

    pie_init_vector_table();
}
...
{
   union PIEIER_REG *pie_regs = (union PIEIER_REG*)&PieCtrlRegs.PIEIER1.all;
   pie_regs[major - 1].all = 1 << (minor - 1);
//   PieCtrlRegs.PIEIER1.bit.INTx1 = ;	// Enable INT 1.1 in the PIE
}
